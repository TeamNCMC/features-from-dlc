...
modality : whiskers
features : deflection left, deflection right
bodyparts : top/left/right nose
author : Guillaume Le Goc (g.legoc@posteo.org) & Alexis d'Humières
version : 2026.07.01
...
PIXEL_SIZE = np.nan  # pixel size in mm
...
BODYPARTS = [
    ...
    "Left_nose",
    "Right_nose",
    "Right_whiskers_middle",
    "Left_whiskers_middle",
    "Right_whiskers_base",
    "Left_whiskers_base",
]
...
FEATURES_NORM = ()#("deflection right", "deflection left")
...
FEATURES_METRICS_RANGE = {
    "deflection right": {"mean": [0.05,0.1], "end":RANGE_END},
    "deflection left": {"mean": [0.05,0.1], "end":RANGE_END},
}
...
FEATURES_METRICS_SHARE = {
    "deflection right": {"mean": True, "end": True, "max": True, "delay": False},
    "deflection left": {"mean": True, "end": True, "max": True, "delay": False},
}
...
FEATURES_LABELS = {
    "deflection right": "whisker deflection contra. (deg)",
    "deflection left": "whisker deflection ispi. (deg)",
}
# Preset y axes limits
FEATURES_YLIM = {"deflection right":[-28,68],
                 "deflection left":[-28,68]} # must be [ymin, ymax], empty {} for automatic
...
class Config:
    ...
    def get_features(self) -> tuple[dict, dict, dict, dict, dict, dict]:
        """
        ...
        # this class file.
        features = {
            "deflection right": lambda df: -self.get_deflection(df, "right"),
            "deflection left": lambda df: self.get_deflection(df, "left"),
        }
        ...
        features_metrics = {
            "deflection right": {"mean": lambda val, _: np.mean(val), "end": lambda val, _: np.mean(val)},
            "deflection left": {"mean": lambda val, _: np.mean(val), "end": lambda val, _: np.mean(val)},
        }
    ...
    def write_parameters_file(
        ...
        with open(os.path.join(outdir, name), "w") as fid:
            ...
            fid.writelines(f"maxrange = '{self.maxrange}'\n")
    ...
    # ----------------------------------------------------------------------------------
    def get_deflection(self, df: pd.DataFrame, side: str) -> pd.Series:
        """
        Whisker angle variation.
        ...
        # get coordinates
        if side == "left":
            whisk_tip = df_dlc["Left_whiskers_middle"][["x", "y"]].to_numpy()
            whisk_base = df_dlc["Left_whiskers_base"][["x", "y"]].to_numpy()
        elif side == "right":
            whisk_tip = df_dlc["Right_whiskers_middle"][["x", "y"]].to_numpy()
            whisk_base = df_dlc["Right_whiskers_base"][["x", "y"]].to_numpy()

        # build vector base_whisker->tip_whisker
        bt_vect = whisk_tip - whisk_base

        # - compute cumulated frame-to-frame angle variation
        # dot product between mn and bt at each time point
        # dot product : x1(t)x2(t) + y1(t)y2(t)
        dotproduct = np.sum(bt_vect[:-1, :] * bt_vect[1:, :], axis=1)  # prop. to cosine
        # determinant between mn and bt at each time point
        x1y2_product = bt_vect[:-1, 0] * bt_vect[1:, 1]
        x2y1_product = bt_vect[1:, 0] * bt_vect[:-1, 1]
