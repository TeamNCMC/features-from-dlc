...
modality : jaw
features : opening, lateralisation
...
PIXEL_SIZE = 2 / 27  # pixel size in mm
...
BODYPARTS = ["Inferior jaw", "Superior jaw"]
...
FEATURES_NORM = ("opening", "lateralisation")
...
MAX_RANGE = 3 # returns max val between (start_stim, start_stim + STIM_DURATION * MAX_RANGE)
...
FEATURES_LABELS = {
    "opening": "opening (mm)",
    "lateralisation": "lateralisation (mm)",
}
# Preset y axes limits
FEATURES_YLIM = {"opening":[-1,1],
                 "lateralisation":[-2,2]}  # must be [ymin, ymax], empty {} for automatic
...
class Config:
    ...
    def get_features(self) -> tuple[dict, dict, dict, dict, dict]:
        ...
        # this class file.
        features = {
            "opening": lambda df: (df["Inferior jaw", "y"] - df["Superior jaw", "y"])
            * self.pixel_size,
            "lateralisation": lambda df: (
                df["Superior jaw", "x"] - df["Inferior jaw", "x"]
            )
            * self.pixel_size,
        }
        ...
        features_metrics = {
            "opening": {"mean": lambda val, _: np.mean(val), "end": lambda val, _: np.mean(val)},
            "lateralisation": {"mean": lambda val, _: np.mean(val), "end": lambda val, _: np.mean(val)},
        }
        ...
        features_metrics_range = {
            "opening": {"mean": [0.05,0.1], "end": RANGE_END},
            "lateralisation": {"mean": [0.05,0.1], "end": RANGE_END},
        }
        ...
        features_metrics_share = {
            "opening": {"mean": True, "end": True, "max": True, "delay": False},
            "lateralisation": {"mean": True, "end": True, "max": True, "delay": False},
        }
