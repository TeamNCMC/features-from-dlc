...
PIXEL_SIZE = np.mean([600 / 1770, 700 / 2079])  # pixel size in mm
...
MAX_RANGE = 2 # returns max val between (start_stim, start_stim + duration_stim * MAXIMUM_RANGE) 
RANGE_END = [1.35 , 1.45] 
...
FEATURES_YLIM =  {"speed": [0,45],
                 "theta_body": [-60,220],
                 "theta_neck": [-40,120],
                 "deceleration":[-100,220]
                     # must be [ymin, ymax]
...
class Config:
    ...
    def get_accel_coef(self, speed, time):
        """
        Compute the acceleration coefficient from the speed time series.

        The acceleration coefficient is approximated by the mean derivative in the selected
        time window. Handles small mismatches in length between speed and time.
        ...
        time : np.ndarray or pd.Series
            (ntimes,), corresponding time vector.
        ...
        if abs(len(speed) - len(time)) > 2:
            raise ValueError(f"Speed and time lengths differ too much: {len(speed)} vs {len(time)}")
    ...
    def get_features(self) -> tuple[dict, dict, dict, dict]:
        ...
        features_metrics = {
            "speed": {
                ...
                "end": lambda val, _: np.mean(val)
            },
            "theta_body": {"mean": lambda val, _: np.mean(val), "end": lambda val, _: np.mean(val)},
            "theta_neck": {"mean": lambda val, _: np.mean(val), "end": lambda val, _: np.mean(val)},
            "xbody": {},
            "ybody": {},
        }
        ...
        features_metrics_range = {
            "speed": {"mean": [0.75, 1], "deceleration": [0.5, 0.60], "end": RANGE_END},
            "theta_body": {"mean": [0.75, 1], "end": RANGE_END},
            "theta_neck": {"mean": [0.75, 1], "end": RANGE_END},
            "xbody": {},
            "ybody": {},
        }
        ...
        features_metrics_share = {
            "speed": {"mean": True, "deceleration": False, "end": True, "min": True, "delay": False},
            "theta_body": {"mean": True, "end": True, "max": True, "delay": False},
            "theta_neck": {"mean": True, "end": True , "max": True, "delay": False},
            "xbody": {},
            "ybody": {},
        }
    ...
    def get_speed(self, df: pd.DataFrame, **kwargs) -> np.ndarray:
        ...
        **kwargs : passed to `speed_bodypart`.
        ...
        return self.speed_bodypart(df_dlc, "centroid", fps,smooth=False, **kwargs)
